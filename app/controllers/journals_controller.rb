class JournalsController < SecureApplicationController
  ...
  # GET /journals.json
  def index
    @journals = Journal.alphabetical.accessible_by(current_ability, :manage).decorate
  end
  ...
  private
    ...
    def journal_params
      params.require(:journal).permit(:name, :description, :public, :password).merge(user: current_user)
    end
end
